...
namespace cobaAI
{
    ...
    class Program
    {
                ...
                if (trainingMode)
                {
                    ...
                    if (isKeyToggled(Keys.End))
                    {
                        ...
                        while (true)
                        {
                            ...
                            else Console.WriteLine("Quando terminar o treinamento digite \"done\" no console.");
                        }
        ...
        public static void PrepareFiles(string game)
        {
            ...
            {
                DialogResult dialogResult = MessageBox.Show($"Parece que está faltando os arquivos principais!(mainfiles)","Mainfiles missing", MessageBoxButtons.YesNo);
                if (dialogResult == DialogResult.Yes)
                Process.GetCurrentProcess().Kill();
            }
        }
